...
class MiniMaxGame:
    ...
    def _minimax_score(self, grid, maximize):
        if self.check_winner_symbol(grid, self.ai_symbol):
            return 1
        if self.check_winner_symbol(grid, self.player_symbol):
            return -1

        if all(cell != self.BLANK for row in grid for cell in row):
            return 0
        ...
        if maximize:
            ...
            for i in range(3):
                for j in range(3):
                    if grid[i][j] == self.BLANK:
                        grid[i][j] = self.ai_symbol
                        score = self._minimax_score(grid, False)
                        grid[i][j] = self.BLANK
                        best = max(best, score)
            return best
            ...
            for i in range(3):
                for j in range(3):
                    if grid[i][j] == self.BLANK:
                        grid[i][j] = self.player_symbol
                        score = self._minimax_score(grid, True)
                        grid[i][j] = self.BLANK
                        best = min(best, score)
            return best

    def do_minimax(self) -> None:
        ...
        for m in self.available_moves():
            r, c = divmod(m - 1, 3)
            self.grid[r][c] = self.ai_symbol
            move_score = self._minimax_score(self.grid, False)
...
def main():
    print("Welcome to Tic-Tac-Toe with AI using MiniMax!")
